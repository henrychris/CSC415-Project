...
class State:
    ...
    def winner(self):
        winner = fbf.check_win(self.board, False)
    ...
    def showBoard(self):
        ...
        print(self.board[8] + " | " + self.board[9] +
              " | " + self.board[10] + " | " + self.board[11])
        print(self.board[12] + " | " + self.board[13] +
...
class AI_minimax:
    def choose_action(self, board):
        fbf.moves_made = 0
        if fbf.moves_made >= 5:
            return fbf.find_best_move(board)
        else:
            return fbf.return_random_move(board)
...
class HumanPlayer:
    ...
    def choose_action(self, positions):
        while True:
            row = int(input("Input a number from 1-16:"))
            action = (row - 1)
